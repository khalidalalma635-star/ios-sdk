...
class ViewController: UIViewController {
    ...
    private func updateViewWithPlayerState(_ playerState: SPTAppRemotePlayerState) {
        ...
        fetchAlbumArtForTrack(playerState.track) { image in
    ...
    private func fetchAlbumArtForTrack(_ track: SPTAppRemoteTrack, callback: @escaping (UIImage) -> Void) {
        appRemote?.imageAPI?.fetchImage(forItem: track, with:CGSize(width: 1000, height: 1000), callback: { image, error in
            guard error == nil else { return }
    ...
    private func unsubscribeFromCapabilityChanges() {
        guard subscribedToCapabilities else { return }
        appRemote?.userAPI?.unsubscribe(toCapabilityChanges: { _, error in
            guard error == nil else { return }
